        ...
        private void LoadRankingData()
        {
            ...
            if (rankings != null && rankings.Count > 0)
            {
               ...
               totalTime // Sử dụng biến totalTime đã thêm chữ "s"
                    ...
                    if (ranking.Id_User == id_User)
                    {
                        KetQuaTable.Rows[i].DefaultCellStyle.BackColor = Color.Orange; 
                    }

                    else if (i == 0)
                    {
                        KetQuaTable.Rows[i].DefaultCellStyle.BackColor = Color.Gold;
                        KetQuaTable.Rows[i].DefaultCellStyle.ForeColor = Color.Black;
                        KetQuaTable.Rows[i].DefaultCellStyle.Font = new Font("Montserrat", 9, FontStyle.Bold);
                    }
                }
            }
            else
