    ...
    internal class ExamModify
    {
        ...
        public List<Ranking> GetRanking()
        {
            ...
            SELECT 
                U.Id_User,
                U.fullname,
                COALESCE(COUNT(DISTINCT HT.Id_Exam), 0) AS Completed_Exams,
                COALESCE(SUM(MAX_Point.Total_Point), 0) AS Total_Points,
            ...
            JOIN 
                (SELECT 
                     Id_Exam,
                     Id_User,
                     MAX(Total_Point) AS Total_Point
                 FROM 
                     History_Test
                 GROUP BY 
                     Id_Exam, Id_User) AS MAX_Point ON HT.Id_Exam = MAX_Point.Id_Exam AND HT.Id_User = MAX_Point.Id_User AND HT.Total_Point = MAX_Point.Total_Point
            GROUP BY 
        ...
        public List<Ranking> GetRankTestBySubject(string subject)
        {
            ...
            U.Id_User, 
            U.fullname, 
            COUNT(DISTINCT HT.Id_Exam) AS Completed_Exams, 
            SUM(MAX_Point.Total_Point) AS Total_Points, 
            SUM(HT.Time_Completed) AS Total_Time 
        FROM 
            Users U 
        JOIN 
            (SELECT 
             ...
             FROM 
                 History_Test 
             GROUP BY 
                 Id_User, Id_Exam) AS MAX_Point ON U.Id_User = MAX_Point.Id_User 
        JOIN 
            History_Test HT ON U.Id_User = HT.Id_User AND HT.Id_Exam = MAX_Point.Id_Exam AND HT.Total_Point = MAX_Point.Total_Point
        WHERE 
            HT.NameExam = @Subject 
        GROUP BY 
            U.Id_User, U.fullname 
